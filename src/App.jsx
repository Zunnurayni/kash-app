...
import { validateAddress as validateAddressLib, qrSvg, watchAddress } from "./bch.js";
...
function validateAddressInline(addr) {
  ...
  // Defer to libauth for true checksum verification when available.
  try {
    const r = validateAddressLib(s);
...
export default function App() {
  ...
  const watchRef = useRef(null);   // demo-fallback timer
  const tickRef = useRef(null);    // seconds counter
  const wsRef = useRef(null);      // live Watchtower socket
  const reconnectRef = useRef(null);

  // ── network config ─────────────────────────────────────────────────────────
  // Watchtower hosted indexer (same infra Paytaca uses). Set DEMO_FALLBACK to
  // false in a real browser/Capacitor build to rely solely on the live socket.
  const WATCHTOWER_WS = "wss://watchtower.cash/ws/watch/bch/";
  const DEMO_FALLBACK = false; // live detection on; set true only to demo without real payments
  ...
  useEffect(() => {
    ...
    // ── REAL PAYMENT DETECTION (Watchtower hosted indexer) ───────────────────
    // Open a websocket scoped to the receiving address. Watchtower pushes a
    // message the instant a transaction paying that address is seen in the
    // mempool (0-conf) — that's the moment we flip to PAID. Structure mirrors
    // Paytaca's src/wallet/transaction-listener.js.
    const addr = address.replace("bitcoincash:", "");
    const url = WATCHTOWER_WS + addr + "/";
    ...
    function connect() {
      try {
        ...
        ws.onmessage = (event) => {
          // Watchtower delivers JSON; shape varies by event, so we read defensively.
          let msg; try { msg = JSON.parse(event.data); } catch { return; }
          // A payment event carries the value sent to this address (in BCH or sats).
          const sats = msg?.value ?? msg?.amount ?? msg?.tx?.value;
          const bchPaid = typeof sats === "number"
            ? (sats > 1e5 ? sats / 1e8 : sats) // tolerate sats- or BCH-denominated
            : null;
          // Accept if it covers the invoice (tiny tolerance for rounding).
          if (bchPaid == null || bchPaid + 1e-8 >= charge.bch) {
            settle({ txid: msg?.txid || msg?.tx?.hash || null });
          }
        };

        ws.onerror = () => setWsStatus("error");
        ws.onclose = () => {
          // auto-reconnect while still waiting on this charge
          if (screen === "charge") {
    ...
    // ── DEMO FALLBACK ────────────────────────────────────────────────────────
    // The in-chat sandbox blocks outbound sockets, so the live connection won't
    // open here. This timer lets you still feel the full flow. In a real build,
    // set DEMO_FALLBACK = false and delete this block.
    if (DEMO_FALLBACK) {
      watchRef.current = setTimeout(() => settle({ txid: "demo" }), 9000);
    }
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.phone}>
        ...
        {screen === "charge" && charge && (
          <Charge
            ...
            secs={secsWaiting} onCancel={reset} onSimulate={() => settle({ txid: "manual" })}
          />
        )}
        {screen === "paid" && charge && (
      ...
      <p style={S.disclaimer}>
        Prototype · non-custodial · funds never pass through this tool. The QR and wallet link
        are real BIP21. Payment detection is wired to Watchtower's live websocket
        (wss://watchtower.cash) — but this in-chat sandbox blocks outbound sockets, so a demo
        timer stands in here. In a real browser/Capacitor build it settles the instant the
        blockchain sees the payment.
      </p>
    </div>
  );
}
...
// ── CHARGE ───────────────────────────────────────────────────────────────────
function Charge({ charge, payUri, secs, wsStatus, onCancel, onSimulate }) {
  ...
  return (
    <div style={S.body}>
      ...
      <a href={payUri} style={S.walletBtn}>
      ...
      <button style={S.simulateBtn} onClick={onSimulate}>▶ Simulate customer paying</button>
...
function QrCode({ text, size = 200 }) {
  ...
  useEffect(() => {
    let alive = true;
    qrSvg(text, { width: size }).then((s) => { if (alive) setSvg(s); });
    return () => { alive = false; };
  }, [text, size]);
  return (
    <div
      style={{ width: size, height: size }}
      aria-label="Scan with any BCH wallet to pay the exact amount"
...
const S = {
  ...
  simulateBtn: { width: "100%", marginTop: 16, background: "#fff", color: "#0F7A3D", border: "1.5px dashed #0F7A3D", borderRadius: 13, padding: "12px 0", fontSize: 13.5, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" },
